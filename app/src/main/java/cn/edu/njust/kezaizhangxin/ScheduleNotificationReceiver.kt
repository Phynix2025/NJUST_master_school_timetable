...
object NotificationScheduler {
    private const val CHANNEL = ScheduleNotificationReceiver.CHANNEL
    ...
    fun ensureChannel(context: Context) {
        val manager = context.getSystemService(NotificationManager::class.java)
        manager.createNotificationChannel(NotificationChannel(CHANNEL, "课程提醒", NotificationManager.IMPORTANCE_HIGH).apply {
            description = "上课前 20 分钟与前一晚的课程提醒"
        })
    }
    ...
    fun schedule(context: Context) {
        ...
        var night = now.withHour(22).withMinute(0).withSecond(0).withNano(0)
        if (!night.isAfter(now)) night = night.plusDays(1)
        set(alarm, context, night.toInstant().toEpochMilli(), ScheduleNotificationReceiver.ACTION_NIGHT, NIGHT_REQUEST_CODE, null)

        cancelPreviouslyScheduledCourses(context, alarm)
        val scheduledIds = mutableSetOf<String>()
        for (offset in 0..14) {
            val date = LocalDate.now().plusDays(offset.toLong())
            coursesOn(context, date).forEachIndexed { index, course ->
                val reminderTime = LocalTime.parse(timeOf(course.section)).minusMinutes(20)
                val whenAt = date.atTime(reminderTime).atZone(ZoneId.systemDefault()).toInstant().toEpochMilli()
                if (whenAt > System.currentTimeMillis()) {
                    val requestCode = (whenAt xor index.toLong()).toInt()
                    set(alarm, context, whenAt, ScheduleNotificationReceiver.ACTION_COURSE, requestCode, "${timeOf(course.section)} 上课 · ${course.text}")
                    scheduledIds += requestCode.toString()
                }
            }
        }
        context.getSharedPreferences(ALARM_PREFS, Context.MODE_PRIVATE)
            .edit().putStringSet(COURSE_IDS, scheduledIds).apply()
    }

    private fun set(alarm: AlarmManager, context: Context, at: Long, action: String, id: Int, message: String?) {
        ...
        if (Build.VERSION.SDK_INT < 31 || alarm.canScheduleExactAlarms()) {
            ...
            // Android 12 users can deny the special access. This still wakes in
            // Doze, although delivery may be delayed until after the target time.
            alarm.setAndAllowWhileIdle(AlarmManager.RTC_WAKEUP, at, pending)
        }
    }

    private fun cancelPreviouslyScheduledCourses(context: Context, alarm: AlarmManager) {
        val prefs = context.getSharedPreferences(ALARM_PREFS, Context.MODE_PRIVATE)
        prefs.getStringSet(COURSE_IDS, emptySet()).orEmpty().forEach { value ->
            val requestCode = value.toIntOrNull() ?: return@forEach
            val pending = PendingIntent.getBroadcast(
                context,
                requestCode,
                Intent(context, ScheduleNotificationReceiver::class.java).setAction(ScheduleNotificationReceiver.ACTION_COURSE),
                PendingIntent.FLAG_NO_CREATE or PendingIntent.FLAG_IMMUTABLE
            )
            if (pending != null) alarm.cancel(pending)
        }
        prefs.edit().remove(COURSE_IDS).apply()
    }
    ...
    private const val NIGHT_REQUEST_CODE = 10
    private const val ALARM_PREFS = "scheduled_alarms"
    private const val COURSE_IDS = "course_request_codes"
}
